...
//Query con l tabla cliente
    $sql = "SELECT * FROM mesero where  fecha BETWEEN '$desde' AND '$hasta' order by idmesero desc";
    ...
    $sql = "SELECT * FROM mesero order by idmesero desc";
...
<td colspan="5"><b>Reporte de Mesero<b>&nbsp;&nbsp;&nbsp;' . $total . '&nbsp;&nbsp;&nbsp;registros</td>
...
if ($total != 0) {
    ...
    foreach ($model as $row) {
        ...
        $html .= '<tr><td>' . $row["idmesero"] . '</td><td>' . $row['nombre'] . " " . $row['apellido'] . '</td>
...
$mpdf->SetTitle("Reporte mesero ".$fecha);
...
$mpdf->Output('Reporte meseros  ' . $fecha . '.pdf', 'I');
