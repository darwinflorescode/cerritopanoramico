...
//Query con l tabla cliente
    $sql = "SELECT produccion.*, producto.nombre as nombrep,producto.estado FROM produccion inner join producto on produccion.idproducto=producto.idproducto where  fechaproduccion BETWEEN '$desde' AND '$hasta' order by idproduccion desc";
    ...
    $sql = "SELECT produccion.*, producto.nombre as nombrep,producto.estado FROM produccion inner join producto on produccion.idproducto=producto.idproducto order by idproduccion desc";
...
<td colspan="5"><b>Reporte de Producci&oacute;n<b>&nbsp;&nbsp;&nbsp;' . $total . '&nbsp;&nbsp;&nbsp;registros</td>
...
if ($total != 0) {
    ...
    foreach ($model as $row) {
        ...
        $sumac += $row['cantidad'];
        $sumast += $row['total'];
        $html .= '<tr><td>' . $row["idproduccion"] . '</td>';
        $html .= '<td>' . date("d-m-Y", strtotime($row['fechaproduccion'])) . '</td>';
        $html .= '<td>' . $row['fechavencimiento'] . '</td>';
        $html .= '<td>' . $row['nombrep'] . '</td>';
        ...
        $html .= '<td style="background-color:#EEEEEE">$' . number_format($row['total'],2) . '</td></tr>';

    }

    $html .= '<tr><td align="right" colspan="4">Totales</td><td style="background-color:#EEEEEE">' . $sumac . '</td><td style="background-color:#EEEEEE">$' .number_format( $suma,2) . '</td><td style="background-color:#EEEEEE">$' . number_format($sumast,2) . '</td></tr>';
...
$mpdf->SetTitle("Reporte producción ".$fecha);
...
$mpdf->Output('Reporte produccion  ' . $fecha . '.pdf', 'I');
