...
//Query con l tabla cliente
        $sql = "SELECT * FROM cliente where  fecha BETWEEN '$desde' AND '$hasta' order by idcliente desc";
...
<table id="ds" width="100%" style="font-size: 10pt; border-collapse: collapse; " cellpadding="7">

<thead>
<tr style="background-color:#EEEEEE;">
<td colspan="5"><b>Reporte de Clientes<b>&nbsp;&nbsp;&nbsp;'.$total.'&nbsp;&nbsp;&nbsp;registros</td>
...
foreach($model as $row)
...

    {
      ...
      $suma +=$row['idcliente'];
        $html .='<tr><td>'.$row["idcliente"].'</td><td>'.$row['nombre']." ".$row['apellido'].'</td>
      ...
      if ($correo!="") {
        ...
        $html .='</td><td>'.$row['direccion'].'</td><td>'.date("d-m-Y", strtotime($row['fecha'])).'</td>';
...
$mpdf->SetTitle("Reporte clientes ".$fecha);
...
$mpdf->Output('Reporte Clientes  '.$fecha.'.pdf','I');
