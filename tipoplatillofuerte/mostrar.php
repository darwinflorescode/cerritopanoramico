...
<!--el siguiente codigo sirve para poder realizar la busqueda en timpo real -->
            <table>
<tr><td>
<input type="text" id="q" name="q"  class="form-control" size="60" onkeyup="load(1);"   placeholder="Busca por ID, nombre platillo,Descripción O opci&oacute;n evento">
<span class="input-group-btn"></td><td>
                                                 
       <button  type="button" class="btn btn-default" onclick="load(1);"  title="Busca por ID, nombre platillo, Descripción O opci&oacute;n evento"><span class="fa fa-search"></span></button>
        </form>
                    </td><td>
...
if (!empty($_GET['save'])) {
  ...
  if ($correcto == "true") {
    ...
    <script>
      function save(){
        alertify.success("Excelente. tipo de platillo fuerte almacenado");
        return false; 
...
<button type="button" class="btn btn-default" data-toggle="modal" data-target="#tipoplatillofuerte_modal" data-placement="left" title="Nuevo Registro!"><i class="fa fa-plus"></i> Nuevo</button>
...
<h3 class="box-title">Listado de Platillos Fuertes</h3>
</div> 
<div class="box-body">
...
<div class="vertabla"></div>
 ...
 <!-- sirve para llamar el archivo viewviewtipoplatillofuerte que es el que contiene las funciones de busqueda -->
 <?php 

 
      require_once('view/viewtipoplatillofuerte.php');
 ...
 //funcion para cargarr los datos
  $(document).ready(function(){
    ...
    function load(page){
      ...
      $("#loader").fadeIn('slow');
      $.ajax({
        url:'view/viewtipoplatillofuerte.php?action=ajax&page='+page+'&q='+q+'&per_page='+per_page,
