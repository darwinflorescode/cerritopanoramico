...
<!--el siguiente codigo sirve para poder realizar la busqueda en timpo real -->
            <table>
<tr><td>
<input type="text" id="q" name="q"  class="form-control" size="60" onkeyup="load(1);"   placeholder="Busca por ID, opción, precio por persona">
<span class="input-group-btn"></td><td>
                                                 
       <button  type="button" class="btn btn-default" onclick="load(1);"  title="Busca por ID, opción, precio por persona"><span class="fa fa-search"></span></button>
        </form>
                    </td><td>
...
if (!empty($_GET['save'])) {
  ...
  if ($correcto == "true") {
    ...
    <script>
      function save(){
        alertify.success("Excelente. Evento Especial almacenado");
        return false; 
...
<button type="button" class="btn btn-default" data-toggle="modal" data-target="#eventosespeciales_modal" data-placement="left" title="Nuevo Registro!"><i class="fa fa-plus"></i> Nuevo</button>
