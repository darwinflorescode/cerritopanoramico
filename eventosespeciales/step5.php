...
if (isset($_POST['accion'])) {
  ...
  $descripcion = $_POST['descripcion'];
 	...
 	$query = $conn->prepare("SELECT  * FROM condiciones where descripcion='$descripcion' and ideventosespeciales='$ideven'");
 	$query->execute();
	$row = $query->rowcount();
       if ($row)
       {
       echo "<script>location.href='step5.php?idevento=".$ideven."&message=error'; </script> ";
       }else{

       	$insertar = $conn->prepare("INSERT INTO condiciones(descripcion,ideventosespeciales) VALUES('".$descripcion."','$ideven')");
       	$insertar->execute();
...
/*
    ...
    echo "<script>location.href='step5.php?idevento=".$ideven."&message=add'; </script> ";
...
<div class="col-md-20">
<form class="form-horizontal" action="step5.php" method="POST" accept-charset="utf-8"   autocomplete="off" role="form" >
...
<li class="active"><a href="step5.php?idevento=<?php echo $_GET['idevento']; ?>" data-toggle="" aria-expanded="false">Condiciones. Paso 5 </a></li>
...
<textarea class="form-control" required name="descripcion" id="descripcion" Placeholder="Ingrese una descripci&oacute;n de condiciones a aplicar en el evento"></textarea>
...
<div class="col-sm-4">
 ...
 <a href="step6.php?idevento=<?php echo $_GET['idevento']; ?>" class="btn btn-info"> Siguiente <span class="fa fa-chevron-right"></span></a>
 ...
 $command = "SELECT * FROM tipoplatillofuerte where ideventosespeciales=".$_GET['idevento']."";
          ...
          <?php  }else{
            ...
            <?php while ($fil = $sqlcomman->fetch()) {
               echo ' <div style="text-align:left; width:90%;">-&nbsp;'.$fil['descripcion'];
               } ?>
          ...
          <?php  }else{
            ...
            <?php while ($fi = $sqlcom->fetch()) {
               echo ' <div style="text-align:left; width:90%;">-&nbsp;'.$fi['descripcion'];
               } ?>
