...
foreach($model as $row)
    ...
    $fechap =$row['fechaproduccion'];
       ...
       <input type="hidden" value="<?php echo $row['nombre'];?>" id="nombrep<?php echo $row['idproduccion'];?>">
        <input type="hidden" value="<?php echo $row['fechavencimiento'];?>" id="fechavencimiento<?php echo $row['idproduccion'];?>">
    <input type="hidden" value="<?php echo $row['idproducto'];?>" id="idpro<?php echo $row['idproduccion'];?>">
    <input type="hidden" value="<?php echo $row['cantidad'];?>" id="cantidad<?php echo $row['idproduccion'];?>">
    <input type="hidden" value="<?php echo $row['preciounitario'];?>" id="preciounitario<?php echo $row['idproduccion'];?>">
    <input type="hidden" value="<?php echo $row['total'];?>" id="total<?php echo $row['idproduccion'];?>">
        ...
        echo "<td width='3%' >";
        //Este es donde se encuentran las acciones a realizar los enlaces de editar,eliminar y egregar cantidad
        echo "
...
?>
<!-- Estos son los lines para edita y agregar cantidad-->

<li><a style='cursor:pointer;' data-toggle='modal' data-target='#modal_update' onclick='editarproduccion("<?php echo $row['idproduccion'];?>");'><i class='fa fa-edit'></i> Editar</a></li>
<li><a style="cursor:pointer;" data-toggle='modal' data-target='#modal_add' onclick='agregarcantidad(<?php echo $row['idproduccion'];?>)'><i class='fa fa-plus-square'></i> Agregar cantidad</a></li>

<?php
//si todo esto es falso entonces no podra editat ni agregar
    }else{

      echo "<li ><a > <i class='fa fa-edit'></i>No puede Editar</a></li>";
      echo "<li ><a > <i class='fa fa-plus-square'></i>No puede Agregar</a></li>";

    }
 
    ?>

<!-- Estos son los lines para borrar un registro -->
<li><a style="cursor:pointer;" onclick='eliminarproduccion(<?php echo $row['idproduccion'];?>)'><i class='fa fa-trash'></i> Borrar</a></li>
