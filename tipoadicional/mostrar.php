...
<!--el siguiente codigo sirve para poder realizar la busqueda en timpo real -->
            <table>
<tr><td>
<input type="text" id="q" name="q"  class="form-control" size="60" onkeyup="load(1);"   placeholder="Busca por ID, Descripci&oacute;n U opci&oacute;n evento" id="q">
<span class="input-group-btn"></td><td>U
...
if (!empty($_GET['save'])) {
  ...
  if ($correcto == "true") {
    ...
    <script>
      function save(){
        alertify.success("Excelente. Tipo adicional almacenado");
        return false; 
...
<button type="button" class="btn btn-default" data-toggle="modal" data-target="#tipoadicional_modal" data-placement="left" title="Nuevo Registro!"><i class="fa fa-plus"></i> Nuevo</button>
...
<h3 class="box-title">Listado Adicional para Eventos </h3>
</div> 
<div class="box-body">
...
<div class="vertabla"></div>
 ...
 <?php 
      require_once('view/viewtipoadicional.php');
 ...
 //funcion para cargarr los datos
  $(document).ready(function(){
    ...
    function load(page){
      ...
      $("#loader").fadeIn('slow');
      $.ajax({
        url:'view/viewtipoadicional.php?action=ajax&page='+page+'&q='+q+'&per_page='+per_page,
