...
<!--el siguiente codigo sirve para poder realizar la busqueda en timpo real -->
            <table>
<tr><td>
<input type="text" id="q" name="q"  class="form-control" size="60" onkeyup="load(1);"   placeholder="Busca por ID,código, nombre, apellido, teléfono, dirección,fecha, mesas o estado" id="q">
<span class="input-group-btn"></td><td>
                                                 
       <button  type="button" class="btn btn-default" onclick="load(1);"  title="Busca por ID,código, nombre, apellido, teléfono, dirección,fecha,mesas o estado"><span class="fa fa-search"></span></button>
                                </form>
                                </td><td>
...
if (!empty($_GET['save'])) {
  ...
  if ($correcto == "true") {
    ...
    <script>
      function save(){
        alertify.success("Excelente. Mesero almacenado");
        return false; 
...
<!-- mustra un mensaje de error ya que el numero de mesa ingresado ya existe -->
    ...
    ?>
    <body onload="tele();"></body>
    <script>
      function tele(){
        alertify.error("Error. El número de teléfono ya existe !");
        return false; 
        //setTimeout('location.href="mostrar.php"', 1500);
      }
    </script>
...
<!-- mustra un mensaje cuando el codigo de mesero ya existe-->
...
<button type="button" class="btn btn-default" data-toggle="modal" data-target="#mesero_modal" data-placement="bottom" title="Nuevo Registro!"><i class="fa fa-plus"></i> Nuevo</button>
...
<h3 class="box-title">Listado de Meseros </h3>
</div> 
<div class="box-body">
...
<div class="vertabla"></div>
 ...
 <?php 
      require_once('view/viewmesero.php');
 ...
 <script>
  ...
  $(document).ready(function(){
    ...
    function load(page){
      ...
      $("#loader").fadeIn('slow');
      $.ajax({
        url:'view/viewmesero.php?action=ajax&page='+page+'&q='+q+'&per_page='+per_page,
