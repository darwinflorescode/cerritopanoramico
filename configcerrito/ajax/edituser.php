...
if (!empty($_FILES["userfile"])) {
    ...
    $image_name    = time() . "_" . basename($_FILES["userfile"]["name"]);
    $target_file   = $image_name;
    $imageFileType = pathinfo($target_file, PATHINFO_EXTENSION);
    $imageFileZise = $_FILES["userfile"]["size"];
    /* Inicio Validacion*/
    ...
    if (((($imageFileType == "jpg") || ($imageFileType == "JPG") || ($imageFileType == "JPEG") || ($imageFileType == "png") || ($imageFileType == "PNG") || ($imageFileType == "jpeg") || $imageFileType == "gif"))) {
        ...
        $image_name = addslashes(file_get_contents($_FILES['userfile']['tmp_name']));

        $conn = conexion();
        //$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql              = "UPDATE perfil SET imgenusers = '$image_name' WHERE idperfil='1';";
        $oo               = $conn->prepare($sql);
        ...
        if ($query_new_insert) {
            ...
            ?>
                        ...
                        <img class="img-responsive" src="data:image/jpg;base64,<?php echo base64_encode($logo); ?>" id="imgas">
                        <?php
