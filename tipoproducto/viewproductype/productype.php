...
//Condicion para verifcarsi se cumple
	if($action == 'ajax'){
		...
		//addcslashes(str, charlist) para no permitir el sql injection
		 ...
		 $aColumns = array('idtipoproducto','nombre','fecha');//Columnas de busqueda de aacuerdo a la base de datos
		 $sTable = "tipoproducto"; // nombre de la tabla
...
$sql = "SELECT * FROM $sTable $sWhere ORDER BY idtipoproducto LIMIT $offset,$per_page";
...
foreach($model as $row)
    {
     ...
     /*   echo "<td>".date("d-m-Y H:i:s", strtotime($row['fecha']))."</td>";*/
        ...
        ?>
        <input type="hidden" value="<?php echo $row['nombre'];?>" id="nombretipoproducto<?php echo $row['idtipoproducto'];?>">

        <?php
        echo "<div class='btn-group pull-right'>
<button type='button' class='btn btn-default dropdown-toggle' data-toggle='dropdown' aria-expanded='false'>Acciones <span class='fa fa-caret-down'></span></button>
<ul class='dropdown-menu'>
<li><a  style='cursor:pointer;' data-toggle='modal' data-target='#modal_updatep' data-placement='bottom' title='Editar Registro' onclick='editartipoproducto(".$row['idtipoproducto'].");''><i class='fa fa-edit'></i> Editar</a></li>";

?>
<li><a style="cursor:pointer;" data-toggle="tooltip" data-placement="bottom" title="Eliminar Registro!" onclick='eliminartipoproducto(<?php echo $row['idtipoproducto'];?>)'><i class='fa fa-trash'></i> Borrar</a></li>
</ul>
</div> 
</td>
