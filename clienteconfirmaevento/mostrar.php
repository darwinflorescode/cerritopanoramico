...
<!--el siguiente codigo sirve para poder realizar la busqueda en timpo real -->
            <table>
<tr><td>
<input type="text" id="q" name="q"  class="form-control" size="60" onkeyup="load(1);"   placeholder="Busca por ID, Precio persona, Cantidad persona, Fecha evento, Cliente, Evento especial">
<span class="input-group-btn"></td><td>
                                                 
       <button  type="button" class="btn btn-default" onclick="load(1);"  title="Busca por ID, Busca por ID, Precio persona, Cantidad persona, Fecha evento, Cliente, Evento especial"><span class="fa fa-search"></span></button>
        </form>
                    </td><td>
...
if (!empty($_GET['save'])) {
  ...
  if ($correcto == "true") {
    ...
    <script>
      function save(){
        alertify.success("Excelente. Confirmación de evento almacenada");
        return false; 
...
<!-- mustra un mensaje de error en el proceso de registro -->
    ...
    <script>
      function error(){
        alertify.error("Lo siento. El evento ya se encuentra asignado al cliente en la fecha estipulada!");
        return false; 
        //setTimeout('location.href="mostrar.php"', 1500);
      }
    </script>
...
<button type="button" class="btn btn-default" data-toggle="modal" data-target="#clienteconfirmaevento_modal" data-placement="left" title="Nuevo Registro!"><i class="fa fa-plus"></i> Nuevo</button>
...
<h3 class="box-title">Cliente confirma evento </h3>
</div> 
<div class="box-body">
...
<div class="vertabla"></div>
 ...
 <!-- sirve para llamar el archivo viewclienteconfirmaevento que es el que contiene las funciones de busqueda -->
 <?php 

 
      require_once('view/viewclienteconfirmaevento.php');
 ...
 //funcion para cargarr los datos
  $(document).ready(function(){
    ...
    function load(page){
      ...
      $("#loader").fadeIn('slow');
      $.ajax({
        url:'view/viewclienteconfirmaevento.php?action=ajax&page='+page+'&q='+q+'&per_page='+per_page,
