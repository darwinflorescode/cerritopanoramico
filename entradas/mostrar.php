...
<!--el siguiente codigo sirve para poder realizar la busqueda en timpo real -->
            <table>
<tr><td>
<input type="text" id="q" name="q"  class="form-control" size="60" onkeyup="load(1);"   placeholder="Busca por ID, Descripción O opci&oacute;n evento" id="q">
<span class="input-group-btn"></td><td>
                                                 
       <button  type="button" class="btn btn-default" onclick="load(1);"  title="Busca por ID, Descripción O opci&oacute;n evento"><span class="fa fa-search"></span></button>
                                </form>
                                </td><td>
...
if (!empty($_GET['save'])) {
  ...
  if ($correcto == "true") {
    ...
    <script>
      function save(){
        alertify.success("Excelente. Entradas de platillo almacenada");
        return false; 
...
<button type="button" class="btn btn-default" data-toggle="modal" data-target="#entradas_modal" data-placement="left" title="Nuevo Registro!"><i class="fa fa-plus"></i> Nuevo</button>
...
<div class="vertabla"></div>
 ...
 <?php 
      require_once('view/viewentradas.php');
 ...
 //funcion para cargarr los datos
  $(document).ready(function(){
    ...
    function load(page){
      ...
      $("#loader").fadeIn('slow');
      $.ajax({
        url:'view/viewentradas.php?action=ajax&page='+page+'&q='+q+'&per_page='+per_page,
